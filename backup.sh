...
TMP_SETUP_CRED=$(mktemp)
TMP_GET_DBS=$(mktemp)
TMP_BACKUP=$(mktemp)
chmod 600 "$TMP_SETUP_CRED" "$TMP_GET_DBS" "$TMP_BACKUP"

trap 'rm -f "$TMP_SETUP_CRED" "$TMP_GET_DBS" "$TMP_BACKUP"' EXIT
...
if ! run_sqlcmd_checked "Credential setup" -S "$SQL_HOST,$SQL_PORT" -U "$SQL_USER" -C -i "$TMP_SETUP_CRED"; then
...
EOF
  ...
  DBS_LIST=$(/opt/mssql-tools/bin/sqlcmd -S "$SQL_HOST,$SQL_PORT" -U "$SQL_USER" -C -W -h -1 -i "$TMP_GET_DBS")
...
GO
EOF
    if run_sqlcmd_checked "Backup of $db" -S "$SQL_HOST,$SQL_PORT" -U "$SQL_USER" -C -i "$TMP_BACKUP"; then
