...
# Allow overriding sqlcmd path for testing
...
# Export password for sqlcmd to use securely (avoids -P flag in process list)
...
if [ "$BACKUP_ALL_DATABASES" = "true" ] || [ "$BACKUP_ALL_DATABASES" = "1" ]; then
  ...
  DBS_LIST=$("$SQLCMD_BIN" -S "$SQL_HOST,$SQL_PORT" -U "$SQL_USER" -C -W -h -1 -i <(cat <<EOF
SET NOCOUNT ON;
