...
# Securely pass password via environment variable to avoid process list exposure
export SQLCMDPASSWORD="$SQL_PASSWORD"

# Securely create temp files
SETUP_CRED_SQL=$(mktemp /tmp/setup_cred_XXXXXX)
GET_DBS_SQL=$(mktemp /tmp/get_dbs_XXXXXX)
BACKUP_SQL=$(mktemp /tmp/backup_XXXXXX)
chmod 600 "$SETUP_CRED_SQL" "$GET_DBS_SQL" "$BACKUP_SQL"
trap 'rm -f "$SETUP_CRED_SQL" "$GET_DBS_SQL" "$BACKUP_SQL"' EXIT
...
if ! run_sqlcmd_checked "Credential setup" -S "$SQL_HOST,$SQL_PORT" -U "$SQL_USER" -C -i "$SETUP_CRED_SQL"; then
