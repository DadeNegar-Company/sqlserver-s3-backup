...
CRED_FILE=$(mktemp /tmp/setup_cred_XXXXXX)
DB_FILE=$(mktemp /tmp/get_dbs_XXXXXX)
BACKUP_FILE=$(mktemp /tmp/backup_XXXXXX)
chmod 600 "$CRED_FILE" "$DB_FILE" "$BACKUP_FILE"
trap 'rm -f "$CRED_FILE" "$DB_FILE" "$BACKUP_FILE"' EXIT
...
if ! run_sqlcmd_checked "Credential setup" -S "$SQL_HOST,$SQL_PORT" -U "$SQL_USER" -C -i "$CRED_FILE"; then
...
EOF
  ...
  DBS_LIST=$(/opt/mssql-tools/bin/sqlcmd -S "$SQL_HOST,$SQL_PORT" -U "$SQL_USER" -C -W -h -1 -i "$DB_FILE")
